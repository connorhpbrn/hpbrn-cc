...
// Shortest signed angle delta from a -> b, in degrees.
...
/**
 * A smooth, responsive custom cursor.
 *
 * - Position follows the pointer with a fast spring.
 * - The arrow rotates to face the direction of travel.
 * - It squash-stretches along the travel axis as speed increases.
 *
 * No external animation library — just hand-written springs + rAF.
 */
export function SmoothCursor() {
  ...
  useEffect(() => {
    ...
    function frame(now: number) {
      ...
      // Sub-step the springs so they stay stable across refresh rates.
      const FIXED = 1 / 360;
      const sub = Math.max(1, Math.ceil(dt / FIXED));
      const h = dt / sub;

      // Update the rotation target from velocity once moving meaningfully.
      // Keep it continuous (no ±180° wrap jumps) by always stepping the target
      // along the shortest arc from where the cursor currently points.
      ...
      // Squash-stretch along the travel axis, frame-rate-independent easing.
      ...
      // the pointer and matches the CSS transform-origin for rotation/scale.
